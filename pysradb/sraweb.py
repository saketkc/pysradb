...
class SRAweb(object):
    ...
    def __init__(self):
        self.base_url = {}
        self.base_url['esummary'] = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi'
        self.base_url['esearch'] = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi'
        self.esearch_params = {}
        self.esearch_params['sra'] = [('db', 'sra'), ('usehistory', 'y'), ('retmode', 'json')]
        self.esearch_params['geo'] = [('db', 'gds'), ('usehistory', 'y'), ('retmode', 'json')]
    ...
    def create_esummary_params(self, esearchresult, db='sra'):
        ...
        return [('query_key', query_key), ('WebEnv', webenv), ('retstart', retstart), ('retmax', retmax)]
    ...
    def sra_metadata(self, srp, 
            ...
            for uid, run_json in runs_json.items():
                ...
                # library_layout
                exp_library_layout = list(exp_library_descriptor['LIBRARY_LAYOUT'].keys())[0]
    ...
    def srp_to_gse(self, srp):
        ...
        for uid in uids:
            ...
            gse_records.append({'experiment_accession': accession, 'experiment_title': title, 'project_accession': srp})
        ...
        for uid in uids:
            ...
            if record['extrelations']:
                ...
                for extrelation in extrelations:
                    ...
                    assert sorted(keys) == sorted(['relationtype', 'targetobject', 'targetftplink'])
                    assert len(values) == 3
    ...
    def gse_to_gsm(self, gse):
        ...
        return gse_df.rename(columns={'accession': 'experiment_alias', 'SRA': 'experiment_accession'})[['project_alias', 'experiment_alias']]
    
    def gse_to_srp(self, gse):
        gse_df = self.fetch_gds_results(gse)
        gse_df = gse_df[gse_df.entrytype == 'GSE']
        return gse_df.rename(columns={'accession': 'project_alias', 'SRA': 'project_accession'})[['project_alias', 'project_accession']]

    
    def gsm_to_srp(self, gsm):
        gsm_df =self.fetch_gds_results(gsm)
        gsm_df = gsm_df[gsm_df.entrytype == 'GSE']
        return gsm_df.rename(columns={'accession': 'experiment_alias', 'SRA': 'project_accession'})[['experiment_alias', 'project_accession']]
