...
def ena_formatted_response():
    return pd.read_csv("./tests/data/test_search/ena_test.csv", dtype=object, na_filter=False)
...
def test_valid_search_query_2_sra(valid_search_input_2):
    ...
    )._format_query_string() == "Escherichia coli AND SRS6898940[Accession] AND Escherichia coli[Organism] AND " \
...
def test_invalid_search_query_1_sra(invalid_search_input_1):
    ...
    )._format_query_string() == "Krypton[Organism]"
...
def test_sra_search_format_result(valid_search_input_1, sra_response_xml, sra_formatted_response):
    query = SraSearch(
        valid_search_input_1["verbosity"],
        valid_search_input_1["return_max"],
        valid_search_input_1["fields"]
    ...
    pd.testing.assert_frame_equal(query.get_df(), sra_formatted_response, check_dtype=False)


# EnaSearch component tests

def test_ena_search_1():
    instance = EnaSearch(0, 100000, {
        "query": ["ribosome", "profiling"],
        "accession": None,
        "organism": None,
        "layout": None,
...
def test_valid_search_query_1_ena(valid_search_input_1):
    assert EnaSearch(
        ...
        valid_search_input_1["return_max"],
        valid_search_input_1["fields"]
    )._format_query_string() == 'experiment_title="*covid-19*" OR (study_accession="COVID-19" OR ' \
...
def test_valid_search_query_2_ena(valid_search_input_2):
    assert EnaSearch(
        ...
        valid_search_input_2["return_max"],
        valid_search_input_2["fields"]
    )._format_query_string() == 'experiment_title="*Escherichia coli*" OR (study_accession="SRS6898940" OR ' \
...
def test_ena_search_format_request(valid_search_input_1):
    query_string = 'experiment_title="*COVID-19*" OR (study_accession="COVID-19" OR ' \
                   'secondary_study_accession="COVID-19" OR sample_accession="COVID-19" OR ' \
                   'secondary_sample_accession="COVID-19" OR experiment_accession="COVID-19" OR ' \
                   'submission_accession="COVID-19" OR run_accession="COVID-19")'
...
def test_ena_search_format_result(valid_search_input_1, ena_response_json, ena_formatted_response):
    query = EnaSearch(
        valid_search_input_1["verbosity"],
        valid_search_input_1["return_max"],
        valid_search_input_1["fields"]
    ...
    pd.testing.assert_frame_equal(query.get_df(), ena_formatted_response, check_dtype=False)
